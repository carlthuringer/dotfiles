...
** DISABLED Ensure that our exec path is configured
   ...
   #+begin_src elisp :noweb-ref nil :no-export yes
     (use-package exec-path-from-shell
       :demand t
       :config
       (exec-path-from-shell-initialize)
       :custom
       (exec-path-from-shell-variables '("GITHUB_USER", "GITHUB_PACKAGE_PULL_TOKEN"))
       )
   #+end_src
...
** Add some homebrew bins on macos
To properly compile the postgresql gem, we need =pg_config= to be on the path.
#+begin_src elisp
  ;; If the operating system is macos, add "/opt/homebrew/opt/libpq/bin" to exec-path
  ;;
  (when (string-equal system-type "darwin")
    (add-to-list 'exec-path "/opt/homebrew/opt/libpq/bin"))
#+end_src

** Use gsed on macos, if it's installed
Man pages can't be read with the macos-bundled version of sed. Let's have it use gsed, or else display a warning.
#+begin_src elisp
  (when (string-equal system-type "darwin")
    (if (executable-find "gsed")
        (setq Man-sed-command "gsed")
      (display-warning 'man-gsed "MacOS detected but `gsed` not found. Use `brew install gnu-sed` to install gsed.")))
...
#+begin_src elisp
  ...
  (use-package consult
    ...
    ;; Configure consult-ripgrep-args to allow ripgrep to search hidden files/folders
...
#+end_src
** [[https://github.com/LionyxML/auto-dark-emacs][Auto Dark Mode]]
Bring emacs into modern realm, with dark mode
#+begin_src elisp
...
:PROPERTIES:
:header-args: :noweb-ref large-language-models
:END:
...
Tried using llm and ellama but it sucks.
...
#+begin_src elisp
  (use-package gptel
    ...
    (gptel-make-anthropic "Claude" :key (gptel-api-key-from-auth-source "api.anthropic.com"))
...
#+begin_src elisp
  (use-package gptel-agent
    :config
    (let ((my-agents-dir (expand-file-name "agents/" user-emacs-directory)))
      (if (file-directory-p my-agents-dir)
          (setq gptel-agent-dirs (list my-agents-dir))
        ;; keep bundled agents but remove dangerous tools
        (setq gptel-agent-dirs (default-value 'gptel-agent-dirs))))
    (gptel-agent-update))
#+end_src
...
A way of interfacing with Model Context Protocol servers with emacs as the client.
  ...
  (use-package mcp
    ...
    :custom (mcp-hub-servers
             `(("fetch" . (:command "docker" :args ("run" "-i" "--rm" "mcp/fetch")))
               ("context7" . (:command "docker" :args ("run" "-i" "--rm" "context7-mcp")))))
...
Integrating using Agent Client Protocol instead of direct API usage.
*So this is basically a wrapper around the claude cli with extra steps. Not worth the trouble*
...
** [[https://github.com/minad/corfu][Corfu]] the autocompleter
Company sucks, corfu is the new hotness with better native completion integrating with emacs. Also company has been breaking down for me lately.
#+begin_src elisp
  (use-package corfu
    :init
    (global-corfu-mode))
  ...
  (use-package emacs
    :custom
    ;; Enable indentation+completion with TAB key instead of M-TAB
    (tab-always-indent 'complete)
    ;; Emacs 30 and newer, Disable ISpell
    (text-mode-ispell-word-completion nil)
    ;; Hide commands in M-x which don't apply to the current mode. Hides Corfu commands
    (read-extended-command-predicate #'command-completion-default-include-p)
    )
#+end_src

** Inline diagnostics (flymake + sideline)
Eglot reports diagnostics through =flymake=, which is built into Emacs. By
default flymake marks the fringe and shows the message in the echo area only
when point is on the offending line. For a VS Code "Error Lens" feel I want the
message to ride the end of the current line, always visible.

[[https://github.com/emacs-sideline/sideline][sideline]] renders annotations in the right margin, and
[[https://github.com/emacs-sideline/sideline-flymake][sideline-flymake]] feeds flymake diagnostics into it. Hooking on =flymake-mode=
means this lights up in every eglot-managed buffer (CUE included) without a
per-language hook. =sideline-flymake-display-mode= set to ='line= shows the
diagnostics for the whole current line rather than only the one under point.

=flymake-no-changes-timeout= is how long flymake waits after a change before
re-checking. For eglot buffers diagnostics are pushed by the server (see
=eglot-send-changes-idle-time= in the Eglot section), so this mainly governs
non-LSP flymake backends; raised to match so nothing rechecks too eagerly.

The delay above governs how soon errors *appear*, but I also want them to
*disappear* the instant I resume typing — not linger for three stale seconds
while I fix them. Flymake has no built-in knob for that asymmetry: its timeouts
only schedule when the next check starts, never clear what's shown. So a small
buffer-local =after-change-functions= hook clears Flymake's diagnostics on every
edit. The fringe, echo area, and sideline all read from Flymake's diagnostic
overlays, so clearing those clears every display at once — no sideline code
involved. I use Flymake's own =flymake--clear-state= rather than deleting
overlays by hand because it also nils the per-backend bookkeeping list, keeping
state consistent for the next server report (which only does a full cleanup on
its *first* report). Fresh diagnostics reappear when the backend next reports,
...
#+begin_src elisp
  (use-package flymake
    ...
    :hook (flymake-mode . crt/flymake-clear-on-edit-setup)
    :preface
    (defun crt/flymake-clear-diagnostics-on-edit (&rest _)
      "Immediately drop shown Flymake diagnostics on any buffer edit.
  They reappear when the backend next reports, so errors clear the
  instant I type but only re-appear after the idle delay."
      (when (hash-table-p flymake--state)
        (maphash (lambda (_backend state) (flymake--clear-state state))
                 flymake--state)))
    (defun crt/flymake-clear-on-edit-setup ()
      "Add or remove the clear-on-edit hook with `flymake-mode'."
      (if flymake-mode
          (add-hook 'after-change-functions
                    #'crt/flymake-clear-diagnostics-on-edit nil t)
        (remove-hook 'after-change-functions
                     #'crt/flymake-clear-diagnostics-on-edit t))))
#+end_src
...
#+begin_src elisp
  (use-package eglot
    ...
    (add-to-list 'eglot-server-programs
                 '(cue-mode . ("cue" "lsp")))
    :custom
    ;; Wait this long after I stop typing before shipping changes to the
    ;; language server. The server only computes diagnostics on what it's been
    ;; told about, so a longer idle time stops it complaining about half-typed
    ;; lines (e.g. "unexpected EOF") while I'm mid-thought. Tradeoff: completion
    ;; / hover triggered inside this window may briefly reflect the pre-edit
    ;; buffer. Bump higher if diagnostics still feel too eager.
    (eglot-send-changes-idle-time 3.0)
    :init
...
** [Disabled] Rest Client
This is a portable, useful way to use emacs as a REST http client.

Disabled <2024-09-13 Fri> because it seems like Verb-mode is the successor.
...
An extension to org mode that supports sending http requests

#+begin_src elisp
  (use-package verb)
#+end_src

Let's also add the org config for it
...
#+begin_src elisp
  (use-package jsonnet-mode
    :mode "\\.\\(lib|j\\)sonnet\\'")
#+end_src
** [[https://github.com/eki3z/mise.el][Mise.el]]
Handles dev tools, runtimes and languages, installation thereof. Better than ASDF.
#+begin_src elisp
  (use-package mise
    :demand t
...
#+end_src
** Emacs Lisp
I want to use lispy-mode when entering an elisp buffer.
#+begin_src elisp
  (use-package lispy
    :hook (emacs-lisp-mode . lispy-mode))
#+end_src
** Markdown Mode
   Soft-wrap prose and center it the same way we do for Org. =visual-line-mode=
   lives in the preloaded built-in =simple.el=, so it needs no package of its own;
   =visual-fill-column-mode= autoloads from the =visual-fill-column= package
   configured under Org-Present.
#+begin_src elisp
  (use-package markdown-mode
    :hook ((markdown-mode . visual-line-mode)
           (markdown-mode . visual-fill-column-mode)))
#+end_src
** [[https://github.com/russell/cue-mode][CUE mode]]
CUE definitions are written with a leading =#= (e.g. =#Renderable=), and
=cue lsp= returns completion candidates that include that =#=. But cue-mode's
syntax table leaves =#= as punctuation, so Emacs computes the completion region
as starting /after/ the =#=. When cycling candidates in Corfu (whose
=corfu-preview-current= inserts the selection inline) the full =#...= candidate
gets dropped in after the existing =#=, doubling it (=tools.##ValuesConfigMap=).
Giving =#= symbol-constituent syntax makes the completion field include the =#=
so candidates replace it cleanly.
#+begin_src elisp
  (use-package cue-mode
    :config
    (modify-syntax-entry ?# "_" cue-mode-syntax-table))
#+end_src
* Org Mode
  :PROPERTIES:
