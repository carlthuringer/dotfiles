...
** Debugging
   ...
   #+begin_src elisp
     ...
     (defun ct/window-visible (b-name)
       "Return whether B-NAME is visible."
       (-> (-compose 'buffer-name 'window-buffer)
           (-map (window-list))
           (-contains? b-name)))

     (defun ct/show-debug-windows (session)
       "Show debug windows."
       (let ((lsp--cur-workspace (dap--debug-session-workspace session)))
         (save-excursion
           ;; display locals
           (unless (ct/window-visible dap-ui--locals-buffer)
             (dap-ui-locals))
           ;; display sessions
           (unless (ct/window-visible dap-ui--sessions-buffer)
             (dap-ui-sessions)))))

     (defun ct/hide-debug-windows (session)
     ...
     (defun ct/enable-dap-modes
         (dap-mode)
                (dap-ui-mode))


     ;; Then set up dap-mode proper

     (use-package dap-mode
       :ensure t
       :no-require t
       :hook ((dap-stopped . ct/show-debug-windows)
              (dap-terminated . ct/hide-debug-windows)
              (dap-session-created . ct/enable-dap-modes))
       )
   #+end_src

*** Ruby Debugging

    First we have to configure =dap-mode= to load up when we load our
    ruby mode in an actual ruby project.

    #+begin_src elisp
      (use-package enh-ruby-mode
        :after dap-mode
        :bind (("C-c d s" . dap-debug)
               ("C-c d h" . dap-hydra))
        :hook dap-mode
        :config (require 'dap-ruby))
    #+end_src

    Must [[https://github.com/rubyide/vscode-ruby/wiki/1.-Debugger-Installation][install]] some rubygems.

    #+begin_src shell
      gem install ruby-debug-ide debase
    #+end_src

    And put them in your Bundle
    ...
    Create a =.dir-locals.el= file in the project directory with a basic
    Debugging configuration. The =:useBundler= piece is a secret
    param. This package is very secret.
        ...
        (let ((project-root (projectile-project-root)))
          (dap-register-debug-template
           ...
           (list :type "Ruby"
                 :cwd project-root
                 :request "launch"
                 :program (concat project-root "/bin/rails")
                 :args '("server")
                 :useBundler t))))
    #+end_src
