...
:END:
Hold the phone. Emacs 27 includes a thing called fido mode. It's like Ido, but not horrible. The only issue is that it's hard to read.

Presently this seems to break the first or second time I invoke it.
...
#+begin_src elisp
  (use-package selectrum :demand t :config (selectrum-mode +1))
#+end_src
...
*** [DISABLED] Swiper/Ivy/Counsel autocompletion frameworks
:PROPERTIES:
:header-args: :tangle nil
:END:
