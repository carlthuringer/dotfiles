...
** Set up MELPA and ORG package archives
   #+begin_src elisp :noweb yes :no-export yes
     (require 'package)
...
** And automatically update packages
   #+begin_src elisp
...
** Org
   ...
   gnu emacs is somewhat old. The latest and greatest org comes from
   the org repository.

   #+begin_src elisp :noweb-ref package-archives
