...
** YaFolding
A folding mode for emacs, use =C-<return>= to fold. [[https://github.com/zenozeng/yafolding.el]]
#+begin_src elisp
...
** LSP mode
   ...
   #+begin_src elisp :noweb-ref nil :noweb yes :no-export yes
   ...
   #+end_src
But wait, elglot is coming in emacs 29! And it's better! Let's try it (disables noweb)   

** [[https://github.com/joaotavora/eglot][Eglot IDE features]]
Eglot is like LSP except less of a mess. And it's native in Emacs 29, coming soon!
#+begin_src elisp
  (use-package eglot
    :hook ((
           dockerfile-mode
