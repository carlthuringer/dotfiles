...
#+begin_src elisp :noweb yes
  (use-package gptel
    :bind (:map gptel-mode-map ("C-x C-g" . gptel-menu))
    :custom
    ...
    (gptel-make-ollama "Ollama"
        ...
        :stream t
        :models '("gemma3:latest" "qwen3:latest" "deepseek-r1:latest" "devstral:latest"))
    <<gptel-tools>>
    )
#+end_src
*** Tools
**** Terraform
#+begin_src elisp :noweb-ref gptel-tools
  (gptel-make-tool
   :name "terraform_init"
   :function (lambda (dir)
               (message "Running: cd %s && terraform init -no-color" dir)
               (let ((output (shell-command-to-string (format "cd %s && terraform init -no-color" dir))))
                 output))  ; Always return the output string
